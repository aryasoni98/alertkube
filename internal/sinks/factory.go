...
// the single source of truth for "which sinks exist": Registry.Names reports
// that set, and app.TestKnownSinksMatchesRegistry pins config.KnownSinks to it
// so routing validation cannot drift from what dispatch can actually reach.
// buildSinks (in the app package) applies per-sink rate overrides on top.
